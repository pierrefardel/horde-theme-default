(function() {
  ...
  var curShape = localStorage.getItem('horde_dev_shape') || 'soft';
  ...
  function applyShape(shape) {
    var h = document.documentElement;
    h.classList.remove('shape-rounded', 'shape-sharp');
    if (shape === 'rounded') h.classList.add('shape-rounded');
    if (shape === 'sharp')   h.classList.add('shape-sharp');
  }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    var sel = 'background:oklch(0.3 0.02 240);color:#fff;border:1px solid oklch(0.5 0.02 240);border-radius:4px;padding:3px 6px;font-size:12px;cursor:pointer';
    ...
    panel.innerHTML = '<span style="font-weight:600;font-size:11px;letter-spacing:.05em;opacity:.7">DEV</span>'
      + '<select id="dev-mode-select" style="' + sel + '">'
      ...
      + '</select>'
      + '<select id="dev-shape-select" style="' + sel + '">'
      + '<option value="soft"'   + (curShape==='soft'   ? ' selected' : '') + '>Soft</option>'
      + '<option value="rounded"'+ (curShape==='rounded'? ' selected' : '') + '>Rounded</option>'
      + '<option value="sharp"'  + (curShape==='sharp'  ? ' selected' : '') + '>Sharp</option>'
      + '</select>';
    ...
    document.getElementById('dev-mode-select').addEventListener('change', function() {
      localStorage.setItem('horde_dev_mode', this.value);
      applyMode(this.value);
    });

    document.getElementById('dev-shape-select').addEventListener('change', function() {
      localStorage.setItem('horde_dev_shape', this.value);
